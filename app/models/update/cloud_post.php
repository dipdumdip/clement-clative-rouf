...
class Cloud_Post extends DatabaseObject {
	//declairing the table name as static
	protected static $table_name="cloud_post";
	//list the essential database fields into an array for CRUD
	protected static $db_fields = array('post_id',  'posting', 'author_id_fk', 'cloud_id_fk', 'ip', 'created', 'uploads');
	...
	public static function get_last_entry_contant($author_id_fk){
	 ...
	 //<----Function to delete recored BY cloud_ID   	
	public static function delete_data_cloud_id($cloud_id_fk=0) {
		$database = new Database;
			$query= "DELETE FROM `".static::$table_name."` WHERE cloud_id_fk = '{$cloud_id_fk}' ";
			$result_array = $database->query($query);
			return $result_array;
	}
    ...
    public static function Delete_Post($uid, $post_id){
		$database = new Database;
		$query = $database->query("DELETE FROM cloud_post WHERE post_id='$post_id' AND author_id_fk='$uid'");
				return true;
	}
