...
class Forum_Post extends DatabaseObject {
	...
	protected static $db_fields = array('post_id',  'posting', 'author_id_fk', 'forum_id_fk', 'ip', 'created', 'uploads', 'share');
	...
	public static function find_the_post_available_CHECK($forum_id_fk=0){
		$result_array = static::find_by_sql("SELECT posting FROM ".static::$table_name." WHERE forum_id_fk='{$forum_id_fk}' LIMIT 1");
	...
	//Delete Post By Forun ID
    public static function Delete_Post_by_forum_id_fk($forum_id_fk){
		$database = new Database;
		$query = $database->query("DELETE FROM ".static::$table_name." WHERE forum_id_fk='{$forum_id_fk}'");
		return true;
	}
